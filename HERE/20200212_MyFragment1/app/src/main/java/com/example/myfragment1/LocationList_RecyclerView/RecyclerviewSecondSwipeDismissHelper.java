...
public class RecyclerviewSecondSwipeDismissHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private List<LocationTagEntity> locationTagEntities;
    ...
    public void onSwiped(@NonNull final RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        locationEntity = locationViewModel.location_delete(recyclerAdapter.getLocationEntityAt(viewHolder.getAdapterPosition()));
        locationTagEntities = locationViewModel.locationTag_FindLocationID(locationEntity.getId());
        
        if(!isEmptyTag){

        }
            final TagEntity[] tagList = locationViewModel.tag_delete(multipleSelectionByForeignKey(locationEntity.getId()).toArray(new TagEntity[0]));
        ...
        Snackbar.make(viewHolder.itemView, "", Snackbar.LENGTH_LONG).setAction("Undo", new View.OnClickListener() {
            ...
            public void onClick(View v) {
                locationViewModel.location_insert(locationEntity);
